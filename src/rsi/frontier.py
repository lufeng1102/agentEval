...
def analyze_frontier(runs_dir: str | Path) -> dict[str, Any]:
    ...
    for report_path in sorted(Path(runs_dir).glob("*/report.json")):
        ...
        for case in report.get("cases", []) or []:
            ...
            item = by_capability.setdefault(capability, {"capability": capability, "best_difficulty": 0, "runs": [], "unsafe_expansion": False})
    ...
    return {"runs_dir": str(runs_dir), "runs": entries, "capabilities": sorted(by_capability.values(), key=lambda item: item["capability"]), "warnings": [f"unsafe capability expansion: {item['capability']}" for item in by_capability.values() if item["unsafe_expansion"]]}
